...
privileged aspect Session_Controller_Roo_Controller {
    ...
    public String Session_Controller.create(@Valid Session_ session_, BindingResult result, Model model, HttpServletRequest request) {
        ...
        session_.persist();
        return "redirect:/session_s/" + encodeUrlPathSegment(session_.getId().toString(), request);
    }
    ...
    public String Session_Controller.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model model) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            model.addAttribute("session_s", Session_.findSession_Entries(page == null ? 0 : (page.intValue() - 1) * sizeNo, sizeNo));
            float nrOfPages = (float) Session_.countSession_s() / sizeNo;
    ...
    public String Session_Controller.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model model) {
        Session_.findSession_(id).remove();
        model.addAttribute("page", (page == null) ? "1" : page.toString());
        model.addAttribute("size", (size == null) ? "10" : size.toString());
        return "redirect:/session_s?page=" + ((page == null) ? "1" : page.toString()) + "&size=" + ((size == null) ? "10" : size.toString());
    }
