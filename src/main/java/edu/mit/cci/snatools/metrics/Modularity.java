package edu.mit.cci.snatools.metrics;
 ...
 *
 * @author pjmcswee
 * Ported to Jung by jintrone <jintrone@mit.edu>
 *
 * Date: 4/20/11
 * Time: 2:19 PM
...
public class Modularity  {
    ...
    class CommunityStructure {
        ...
        HashMap<DefaultJungNode, Integer> map;
        ...
        CommunityStructure(Graph<DefaultJungNode,DefaultJungEdge> hgraph) {
            ...
            for (DefaultJungNode node : hgraph.getVertices()) {
                ...
                for (DefaultJungNode neighbor : hgraph.getNeighbors(node)) {
                    if (node == neighbor) {
                        continue;
                    }
    ...
    public void execute(Graph<DefaultJungNode,DefaultJungEdge> hgraph) {
        isCanceled = false;
        ...
        int[] comStructure = new int[hgraph.getVertexCount()];
        int count = 0;
        ...
        for (DefaultJungNode node : hgraph.getVertices()) {
            ...
            degreeCount[comStructure[index]] += hgraph.getNeighborCount(node);
        }

        modularity = finalQ(comStructure, degreeCount, hgraph);
    }

    private double finalQ(int[] struct, double[] degrees, Graph<DefaultJungNode,DefaultJungEdge> hgraph) {


        double res = 0;
        double[] internal = new double[degrees.length];
        for (DefaultJungNode n : hgraph.getVertices()) {
            int n_index = structure.map.get(n);
            n.setAttribute(MODULARITY_CLASS, "Modularity Class "+struct[n_index]);
            for (DefaultJungNode neighbor : hgraph.getNeighbors(n)) {
                if (n == neighbor) {
                    continue;
                }
        ...
        for (int i = 0; i < degrees.length; i++) {
            internal[i] /= 2.0;
            res += (internal[i] / hgraph.getEdgeCount()) - Math.pow(degrees[i] / (2 * hgraph.getEdgeCount()), 2);
        }
        return res;
    }
